...
use PhpWord\Escaper\RegExp;
use PhpWord\Escaper\Xml;
use PhpWord\Exception\CopyFileException;
use PhpWord\Exception\CreateTemporaryFileException;
use PhpWord\Exception\Exception;
use PhpWord\Shared\Text;
use PhpWord\Shared\XMLWriter;
use PhpWord\Shared\ZipArchive;
...
class TemplateProcessor
{
     ...
     *
     * @return \PhpWord\Shared\ZipArchive
     */
    public function zip()
    {
    ...
    /**
     * @param string $search
     * @param \PhpWord\Element\AbstractElement $complexType
     */
    public function setComplexValue($search, \PhpWord\Element\AbstractElement $complexType)
    {
        $elementName = substr(get_class($complexType), strrpos(get_class($complexType), '\\') + 1);
        $objectClass = 'PhpOffice\\PhpWord\\Writer\\Word2007\\Element\\' . $elementName;

        $xmlWriter = new XMLWriter();
        /** @var \PhpWord\Writer\Word2007\Element\AbstractElement $elementWriter */
     ...
     * @param \PhpWord\Element\AbstractElement $complexType
     */
    public function setComplexBlock($search, \PhpWord\Element\AbstractElement $complexType)
    {
        $elementName = substr(get_class($complexType), strrpos(get_class($complexType), '\\') + 1);
        $objectClass = 'PhpOffice\\PhpWord\\Writer\\Word2007\\Element\\' . $elementName;
     ...
     * @param \PhpWord\Element\AbstractElement $complexType
     */
    public function setChart($search, \PhpWord\Element\AbstractElement $chart)
    {
        ...
        // Get the part writer
        $writerPart = new \PhpWord\Writer\Word2007\Part\Chart();
    ...
    public function pasteBlock($blockname, $matches, $indexVariables = false, $variableReplacements = null){
        $wherePaste = $this->getBlockbyName($blockname, $this->tempDocumentMainPart);
        if (isset($matches[3])) {
            $xmlBlock = $matches[2];
            if($indexVariables) {
                foreach ($variableReplacements as $key => $value) {
                    $whereChangeVariable = $this->getBlockbyName($key, $xmlBlock);
                    ...
                    if(!is_array($value)){
                        ...
                        foreach ($value as $k => $v) {
                            $local = $whereChangeVariable[2];
                            foreach($v as $chiave => $valore){
                                if(is_array($valore)){
                                    $local = $this->recorsivePaste($local, $chiave, $valore);
                                }
                                else{
                                    $local = str_replace(
                                        '${'. $chiave."}",
                        ...
                        $xmlBlock = str_replace(
                            $whereChangeVariable[1] . $whereChangeVariable[2] . $whereChangeVariable[3],
                            $xmlBlockLocal,
                            $xmlBlock
                        );
                    }
            ...
            $this->tempDocumentMainPart = str_replace(
                $wherePaste[1] . $wherePaste[2] . $wherePaste[3],
                $wherePaste[1].$wherePaste[2].$xmlBlock.$wherePaste[3],
    ...
    public function ripulisci($blockname){
        $this->tempDocumentMainPart = str_replace('${' . $blockname . '}','', $this->tempDocumentMainPart);
        $this->tempDocumentMainPart = str_replace('${/' . $blockname . '}','', $this->tempDocumentMainPart);
    }
    private function recorsivePaste($xmlblock, $key, $array){
        $whereChangeVariable = $this->getBlockbyName($key, $xmlblock);
        ...
        foreach($array as $chiave => $valore){
            ...
            foreach($valore as $k => $v){
                if(is_array($v)){
                    $local = $this->recorsivePaste($xmlblock, $k, $v);
                }
                else{
                    $local = str_replace(
                        '${'. $k."}",
    ...
    public function getBlockbyName($blockname, $xml){
        $wherePaste = null;
        preg_match(
    ...
    /**
     * la funzione effettua come il taglia e ritorna quello trovato
     * @param $blockname
     * @return array
     *
     */
    public function copyBlock($blockname){
        $matches = array();
        preg_match(
            '/(\${' . $blockname . '})(.*?)(\${\/' . $blockname . '})/is',
            $this->tempDocumentMainPart,
     ...
     * @return \PhpWord\TemplateProcessor Fluent interface
     */
