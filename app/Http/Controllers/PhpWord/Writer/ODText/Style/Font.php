...
namespace PhpWord\Writer\ODText\Style;
...
class Font extends AbstractStyle
{
    ...
    {
        $style = $this->getStyle();
        if (!$style instanceof \PhpWord\Style\Font) {
            return;
        }
        $xmlWriter = $this->getXmlWriter();

        $stylep = (method_exists($style, 'getParagraph')) ? $style->getParagraph() : null;
        if ($stylep instanceof \PhpWord\Style\Paragraph) {
            $temp1 = clone $stylep;
            $temp1->setStyleName($style->getStyleName());
            $temp2 = new \PhpWord\Writer\ODText\Style\Paragraph($xmlWriter, $temp1);
            $temp2->write();
        }
        ...
        $xmlWriter->writeAttributeIf($color != '', 'fo:color', '#' . \PhpWord\Shared\Converter::stringToRgb($color));
