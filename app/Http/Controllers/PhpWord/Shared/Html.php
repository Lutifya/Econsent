...
namespace PhpWord\Shared;

use PhpWord\Element\AbstractContainer;
use PhpWord\Element\Row;
use PhpWord\Element\Table;
use PhpWord\Settings;
use PhpWord\SimpleType\Jc;
use PhpWord\SimpleType\NumberFormat;
...
class Html
{
     ...
     *
     * @param \PhpWord\Element\AbstractContainer $element Where the parts need to be added
                ...
                switch (strtolower($attribute->name)) {
                    ...
                    case 'width':
                        ...
                        if (false !== strpos($val, '%')) {
                            ...
                            $styles['unit'] = \PhpWord\SimpleType\TblWidth::PERCENT;
                        } else {
                            // e.g. <table width="250> where "250" = 250px (always pixels)
                            $styles['width'] = Converter::pixelToTwip($val);
                            $styles['unit'] = \PhpWord\SimpleType\TblWidth::TWIP;
                        }
                        break;
                    case 'cellspacing':
     ...
     * @param \PhpWord\Element\AbstractContainer $element object to add an element corresponding with the node
     ...
     *
     * @param \DOMNode $node
     * @param \PhpWord\Element\AbstractContainer $element
     * @param array $styles
     * @param array $data
     */
     ...
     * @param \PhpWord\Element\AbstractContainer $element
     * @param array &$styles
     * @return \PhpWord\Element\TextRun
     */
     ...
     *
     * @param \DOMNode $node
     * @param \PhpWord\Element\Table $element
     * @param array &$styles
     * @return Row $element
     */
     ...
     * @param \PhpWord\Element\Table $element
     * @param array &$styles
     * @return \PhpWord\Element\Cell|\PhpWord\Element\TextRun $element
     */
    ...
    protected static function parseList($node, $element, &$styles, &$data)
    {
        ...
        if (isset($data['listdepth'])) {
            ...
            /** @var \PhpWord\Style\NumberingLevel */
            $level = $levels[0];
            if ($start > 0) {
        ...
        foreach ($properties as $property) {
            ...
            switch ($cKey) {
                ...
                case 'line-height':
                    $matches = array();
                    if ($cValue === 'normal') {
                        $spacingLineRule = \PhpWord\SimpleType\LineSpacingRule::AUTO;
                        $spacing = 0;
                    } elseif (preg_match('/([0-9]+\.?[0-9]*[a-z]+)/', $cValue, $matches)) {
                        //matches number with a unit, e.g. 12px, 15pt, 20mm, ...
                        $spacingLineRule = \PhpWord\SimpleType\LineSpacingRule::EXACT;
                ...
                case 'width':
                    if (preg_match('/([0-9]+[a-z]+)/', $cValue, $matches)) {
                        ...
                        $styles['unit'] = \PhpWord\SimpleType\TblWidth::AUTO;
                    }
                    break;
                case 'border':
                case 'border-top':
        ...
        foreach ($node->attributes as $attribute) {
            switch ($attribute->name) {
                ...
                case 'width':
                    ...
                    $style['width'] = $width;
                    $style['unit'] = \PhpWord\Style\Image::UNIT_PX;
                    break;
                case 'height':
                    ...
                    foreach ($styleattr as $attr) {
                        if (strpos($attr, ':')) {
                            ...
                            switch ($k) {
                                case 'float':
                                    if (trim($v) == 'right') {
                                        $style['hPos'] = \PhpWord\Style\Image::POS_RIGHT;
                                        $style['hPosRelTo'] = \PhpWord\Style\Image::POS_RELTO_MARGIN; // inner section area
                                        $style['pos'] = \PhpWord\Style\Image::POS_RELATIVE;
                                        $style['wrap'] = \PhpWord\Style\Image::WRAP_TIGHT;
                                        $style['overlap'] = true;
                                    }
                                    if (trim($v) == 'left') {
                                        $style['hPos'] = \PhpWord\Style\Image::POS_LEFT;
                                        $style['hPosRelTo'] = \PhpWord\Style\Image::POS_RELTO_MARGIN; // inner section area
                                        $style['pos'] = \PhpWord\Style\Image::POS_RELATIVE;
                                        $style['wrap'] = \PhpWord\Style\Image::WRAP_TIGHT;
                                        $style['overlap'] = true;
