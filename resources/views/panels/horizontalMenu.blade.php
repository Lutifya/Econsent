      ...
      <ul class="nav navbar-nav flex-row">
        <li class="nav-item mr-auto"><a class="navbar-brand" href="{{url('/')}}"><span class="brand-logo">
          <svg viewbox="0 0 139 95" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="24">
            ...
            <h2 class="brand-text mb-0">Econsent</h2></a></li>
      ...
      <ul class="nav navbar-nav" id="main-menu-navigation" data-menu="menu-navigation">
        ...
        @if(isset($menuData[1]))
          @foreach($menuData[1]->menu as $menu)
            @if(!isset($menu["navheader"]) and $menu['hideHorizontal'] === '')
              @php
                $custom_classes = "";
              ...
              @if(array_search("guest", $roles) !== false or Auth::user() !== null and array_search(Auth::user()->role, $roles) !== false or Auth::user() !== null and Auth::user()->role === "admin")
                ...
                @if(isset($menu["submenu"])){{'data-menu=dropdown'}}@endif>
                  <a href="{{(isset($menu["url"]) and !isset($menu["SubMenu"])) ? url($menu["url"]):'javascript:void(0)'}}" class="nav-link d-flex align-items-center @if(isset($menu["SubMenu"])){{'dropdown-toggle'}}@endif" target="{{isset($menu["newTab"]) ? '_blank':'_self'}}"  @if(isset($menu["SubMenu"])){{'data-toggle=dropdown'}}@endif>
